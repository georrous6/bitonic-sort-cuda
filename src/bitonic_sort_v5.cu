...
__global__
static void kernel_compare_and_swap_v5(int *data, int size, int step, int log2step)
{
...
__global__ 
static void kernel_intra_block_sort_v5(int *data, int chunk_size) {
...
__global__
static void kernel_intra_block_refine_v5(int *data, int size, int chunk_size) {
...
int bitonic_sort_v5(int *host_data, int n, int descending) {
    ...
    // Intra block sorting
    kernel_intra_block_sort_v5<<<numBlocks, threadsPerBlock, shared_mem_block_bytes>>>(device_data, chunk_size);
    ...
    for (int size = chunk_size << 1; size <= n; size <<= 1) {
        ...
        for (int step = size >> 1; step > max_step; step >>= 1) {
            ...
            kernel_compare_and_swap_v5<<<numBlocks, threadsPerBlock>>>(device_data, size, step, log2step);
            if (post_launch_barrier_and_check()) {
                cudaFree(device_data);
                return EXIT_FAILURE;
            }
        ...
        kernel_intra_block_refine_v5<<<numBlocks, threadsPerBlock, shared_mem_block_bytes>>>(device_data, size, chunk_size);
        if (post_launch_barrier_and_check()) {
            cudaFree(device_data);
            return EXIT_FAILURE;
        }
